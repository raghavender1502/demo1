from .GithubSecretMain import getDataFromKeyword
...
async def search_in_githubsecretdomain(domain):
    MongoDBName = os.getenv("MongoDBName")
    CollectionName = os.getenv("GithubSecretCollection")
...
async def store_in_githubsecretdomain(data):
...
def GitHubSecretDomainChecks(domain):
    try:
        try:
            loop = asyncio.new_event_loop()
            data = loop.run_until_complete(search_in_githubsecretdomain(domain))
        ...
        if(data):
            ...
            data = getDataFromKeyword(domain)
            if len(data["TestResults"])==0:
                return data
            else:
                try:
                    loop = asyncio.new_event_loop()
                    loop.run_until_complete(store_in_githubsecretdomain(data))
                except:
